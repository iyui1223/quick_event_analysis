#!/usr/bin/env python3
"""
Heat influx histogram: (westerly wind) × (temperature above 0°C) over Antarctic Peninsula.
- Represents Föhn-driven heat transport: westerly winds bringing warm air.
- Reads CDS download: YYYY02.nc (zip with u10, v10, t2m .nc inside).
- For each day: heat_influx = mean(u10 * t2m where t2m > 0°C) over land+ice shelves.
- Past: 1948–1987, Present: 1988–current. Stacked histograms by decade.
"""
import os
import zipfile
import tempfile
...
LAND_THRESHOLD = 0.5
K2C = 273.15
T_THRESHOLD_C = 0.0  # Only count where T > 0°C
...
def _find_u_t2m_nc(dir_path):
    """Return (path_to_u_nc, path_to_t2m_nc) from extracted files."""
    files = [f for f in os.listdir(dir_path) if f.endswith(".nc")]
    u_path = t2m_path = None
    for f in files:
        if "10m_u" in f and "wind" in f:
            u_path = os.path.join(dir_path, f)
        elif "2m_temperature" in f or ("temperature" in f.lower() and "2m" in f.lower()):
            t2m_path = os.path.join(dir_path, f)
    return u_path, t2m_path
...
def load_heat_influx_for_month(zip_path, land_mask):
    """
    Unzip zip_path, load u10 and t2m, compute daily mean heat influx over land.
    Heat influx = u10 * t2m where t2m > 0°C (westerly × warm temperature).
    Return 1D array of length n_days (mean heat influx per day).
    """
    with tempfile.TemporaryDirectory() as tmp:
        _unzip_month(zip_path, tmp)
        u_path, t2m_path = _find_u_t2m_nc(tmp)
        if u_path is None or t2m_path is None:
            return None
        du = xr.open_dataset(u_path)
        dt = xr.open_dataset(t2m_path)
        # Variable names: u10, t2m
        ...
        t2m_c = t2m - K2C
        # Heat influx: u10 * t2m where t2m > 0°C
        # Only count positive u (westerly) and positive T
        heat = u.values * np.where(t2m_c.values > T_THRESHOLD_C, t2m_c.values, np.nan)
        # land_mask: (41, 201) lat -65..-75, same order as CDS
        land = land_mask
        # heat: (valid_time, latitude, longitude) -> for each time, mask by land and take mean
        land_broadcast = np.broadcast_to(land, heat.shape)
        masked = np.where(land_broadcast, heat, np.nan)
...
def collect_all_daily_means(data_dir, land_mask):
    ...
    for path in sorted(glob.glob(pattern)):
        ...
        if not zipfile.is_zipfile(path):
            continue
        arr = load_heat_influx_for_month(path, land_mask)
        if arr is not None:
            results[year] = arr
    return results
...
def main():
    ...
    if all_vals.size == 0:
        ...
        return
    # Filter out NaN/negative values (days with no warm westerly conditions)
    all_vals = all_vals[~np.isnan(all_vals)]
    all_vals = all_vals[all_vals > 0]  # Only positive heat influx
    if all_vals.size == 0:
        print("No positive heat influx values found.")
        return
    bins = np.linspace(0, np.nanmax(all_vals) * 1.05, 25)
    ...
    # Past: stacked by decade (filter NaN/negative per decade)
    past_data = []
    for k in sorted(past_by_decade.keys()):
        arr = np.array(past_by_decade[k])
        arr = arr[~np.isnan(arr)]
        arr = arr[arr > 0]
        past_data.append(arr.tolist())
    ...
    ax1.set_xlabel("Heat influx [m/s × °C]")
    ax1.set_ylabel("Count")
    ax1.legend(loc="best", fontsize=8)
    
    # Present: stacked by decade
    present_data = []
    for k in sorted(present_by_decade.keys()):
        arr = np.array(present_by_decade[k])
        arr = arr[~np.isnan(arr)]
        arr = arr[arr > 0]
        present_data.append(arr.tolist())
    ...
    ax2.set_xlabel("Heat influx [m/s × °C]")
    ax2.legend(loc="best", fontsize=8)
    
    fig.suptitle("February: Föhn heat influx (westerly × T>0°C) over peninsula — by decade")
    plt.tight_layout()
    out = os.path.join(FIGS_DIR, "peninsula_heat_influx_histogram_past_present.png")
