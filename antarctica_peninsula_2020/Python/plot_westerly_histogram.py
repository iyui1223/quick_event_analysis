#!/usr/bin/env python3
"""
Histogram of mean westerly wind component (u10) over Antarctic Peninsula (land only), February.
- Reads CDS download: YYYY02.nc (zip with u10, v10, t2m .nc inside).
- Masks sea using ERA5 land-sea mask; for each day takes mean westerly wind (u10) over land.
- Past: 1948–1987, Present: 1988–current. Stacked histograms by decade.
- Positive u10 = westerly (west to east), negative = easterly.
"""
import os
import zipfile
import tempfile
...
def _find_u_nc(dir_path):
    """Return path to u10 NetCDF from extracted files."""
...
def load_land_mask():
    ...
    if "longitude" in lsm.dims:
        ...
        lsm = lsm.sel(lat=slice(-65, -75), lon=slice(-90, -40))
    land = (lsm.values >= LAND_THRESHOLD).squeeze()
    ds.close()
    return land


def load_westerly_for_month(zip_path, land_mask):
    """
    Unzip zip_path, load u10, compute daily mean westerly wind over land.
    Return 1D array of length n_days (mean westerly per day).
    """
    with tempfile.TemporaryDirectory() as tmp:
        _unzip_month(zip_path, tmp)
        u_path = _find_u_nc(tmp)
        if u_path is None:
            return None
        du = xr.open_dataset(u_path)
        # Variable name: u10; dims valid_time, latitude, longitude
        u = du["u10"] if "u10" in du else du[list(du.data_vars)[0]]
        # land_mask: (41, 201) lat -65..-75, same order as CDS
        land = land_mask
        # u: (valid_time, latitude, longitude) -> for each time, mask by land and take mean
        land_broadcast = np.broadcast_to(land, u.values.shape)
        masked = np.where(land_broadcast, u.values, np.nan)
...
def collect_all_daily_means(data_dir, land_mask):
    ...
    for path in sorted(glob.glob(pattern)):
        ...
        if not zipfile.is_zipfile(path):
            continue
        arr = load_westerly_for_month(path, land_mask)
        if arr is not None:
            results[year] = arr
    return results
...
def main():
    ...
    if all_vals.size == 0:
        ...
        return
    # Bins: include negative (easterly) and positive (westerly)
    ...
    ax1.set_xlabel("Mean westerly wind over peninsula land [m/s]")
    ...
    ax2.set_xlabel("Mean westerly wind over peninsula land [m/s]")
    ax2.legend(loc="upper right", fontsize=8)
    ax2.axvline(x=0, color="gray", linestyle="--", linewidth=0.5, alpha=0.5)
    
    fig.suptitle("February: ERA5 10m westerly wind component (land-masked peninsula) — by decade")
    plt.tight_layout()
    out = os.path.join(FIGS_DIR, "peninsula_westerly_histogram_past_present.png")
